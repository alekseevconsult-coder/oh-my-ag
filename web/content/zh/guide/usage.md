...
2. **技能会自动检测。** Antigravity 扫描 `.agents/skills/` 并索引所有可用技能。
...
**执行过程：**
- Antigravity 检测到匹配 `frontend-agent`
- 技能自动加载（渐进式披露）
...
### 渐进式披露
Antigravity 自动将请求匹配到技能。您无需手动选择技能。只有所需的技能会加载到上下文中。
...
| 技能 | 自动激活场景 | 输出 |
|------|-------------|------|
| workflow-guide | 复杂的多领域项目 | 分步代理协调 |
...
| qa-agent | "review security"、"audit" | 带优先级修复建议的 QA 报告 |
| debug-agent | 缺陷报告、错误消息 | 修复后的代码、回归测试 |
| orchestrator | CLI 子代理执行 | 结果保存在 `.agents/results/` |
| commit | "commit"、"커밋해줘" | Git 提交（按功能自动拆分） |

---

## 工作流命令

在 Antigravity IDE 聊天中输入以下命令以触发分步工作流：
...
这些与 **技能**（自动激活）是分开的。工作流让您对多步骤流程拥有显式控制权。
...
您："Build a TODO app with authentication"
  → workflow-guide 自动激活
  → PM Agent 创建规划
  → 您通过 CLI 启动代理（oh-my-ag agent:spawn）
  → 代理并行工作
  → QA Agent 评审
  → 修复问题，迭代
```
...
终端 2：oh-my-ag agent:spawn backend "task" session-01 &
...
| 问题 | 解决方案 |
|------|----------|
| 技能未在 Antigravity 中加载 | 使用 `antigravity open .` 打开项目，验证 `.agents/skills/` 和 `SKILL.md`，然后重启 Antigravity IDE |
...
如果您想将这些技能集成到现有的 Antigravity 项目中，请参阅[集成到现有项目](./integration.md)了解：
- 快速三步集成
- 完整仪表盘集成
- 为您的技术栈定制技能
- 故障排除与最佳实践
...
**直接在 Antigravity IDE 中聊天即可。** 如需监控，使用仪表盘。如需 CLI 执行，使用编排器脚本。如需集成到现有项目，请参阅[集成到现有项目](./integration.md)。
