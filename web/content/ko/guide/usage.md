...
2. **스킬이 자동으로 감지됩니다.** Antigravity가 `.agents/skills/`를 스캔하여 모든 스킬을 인덱싱합니다.
...
| pm-agent | "기획해줘", "분석해줘" | `.agents/plan.json` |
...
| orchestrator | CLI 서브에이전트 실행 | `.agents/results/`에 결과 저장 |
...
| 문제 | 해결법 |
|------|--------|
| Antigravity에서 스킬이 로드되지 않음 | `antigravity open .`으로 프로젝트를 열고, `.agents/skills/` 폴더와 `SKILL.md` 파일을 확인한 뒤 Antigravity IDE를 재시작하세요 |
